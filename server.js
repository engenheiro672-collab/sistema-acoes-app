...
async function getPrevendaPublicData(slug) {
  ...
  const { data: prevenda } = await supabase.from('prevendas').select('*, sorteios(slug, nome, foto_url, fotos_galeria, pixel_fb_override, total_cotas, cotas_vendidas, marca_checkout_nome, marca_checkout_descricao, marca_checkout_foto_url), funis(slug)').eq('slug', slug).eq('ativo', true).maybeSingle();
  ...
  const dados = {
    prevenda: {
      ...
      // ⚡ Só usados pela Pré-venda 010 (vídeo de fundo colado por link + foto de capa da tela de
      // carregamento) — nas outras prévendas isso fica vazio e não muda nada do comportamento delas.
      video_url: prevenda.video_url || '', imagem_capa_url: prevenda.imagem_capa_url || ''
    },
    ...
    sorteio: {
      slug: prevenda.sorteios?.slug || null, nome: prevenda.sorteios?.nome || '', foto_url: fotoEscolhida,
      percentual_vendido: percentualVendido,
      marca_nome: prevenda.sorteios?.marca_checkout_nome || '', marca_descricao: prevenda.sorteios?.marca_checkout_descricao || '', marca_foto_url: prevenda.sorteios?.marca_checkout_foto_url || ''
    },
...
app.get('/prevenda/:slug', async (req, res) => {
  try {
    ...
    const temaParaArquivo = { claro: 'prevenda-clara.html', teste: 'prevenda-teste.html', simples: 'prevenda-simples.html', avancar: 'prevenda-avancar.html', '010': 'prevenda-010.html' };
...
app.put('/api/admin/prevendas/:id', ensureAdminAuth, async (req, res) => {
  try {
    ...
    if (body.tema !== undefined) payloadPrevenda.tema = ['claro', 'teste', 'direto', 'simples', 'avancar', '010'].includes(body.tema) ? body.tema : 'escuro';
